...
const SignUp: React.FC = () => {
  ...
  return (
    // FIXED: Switched public asset import reference to a direct root asset path string
    <div className="fixed inset-0 w-screen h-screen flex items-center justify-center bg-cover bg-center bg-no-repeat overflow-hidden" style={{ backgroundImage: "url('/assets/Background.jpg')" }}>
      ...
      <div className="z-10 w-[90%] max-w-sm md:max-w-md bg-gray-900/90 flex flex-col items-center gap-6 p-8 rounded-2xl shadow-2xl border border-white/10">
        {/* FIXED: Switched public asset import reference to a direct root asset path string */}
        <img src="/assets/shesh.png" alt="Logo" className="w-20 h-auto object-contain" />
        <h1 className="text-2xl font-bold text-white uppercase tracking-wider">Sign Up</h1>
        ...
        <form onSubmit={handleSignUp} className="w-full flex flex-col gap-4">
          ...
          <div className="w-full flex items-center bg-gray-800/80 p-3 rounded-lg gap-3 relative border border-transparent focus-within:border-blue-500 transition-all">
          ...
          <div className={`w-full flex items-center bg-gray-800/80 p-3 rounded-lg gap-3 relative border ${confirmPassword && password !== confirmPassword ? 'border-red-500' : 'border-transparent'}`}>
            <FaFingerprint className="text-gray-400" />
            <input type={showPassword ? "text" : "password"} placeholder="Confirm Password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="bg-transparent w-full outline-none text-white text-sm" />
          ...
          <button type="submit" disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-3 rounded-lg mt-2 active:scale-95 transition-all uppercase text-xs tracking-widest shadow-lg">
